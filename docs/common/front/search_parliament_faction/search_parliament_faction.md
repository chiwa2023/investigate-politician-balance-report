...
| 1    | 新規行追加                   | ボタン             | 活性<br>表示／非表示 | 押下時：議会内会派検索結果テーブルの最終に新しい行を追加し、その行を選択状態とすること |
...
- キャンセルイベント送信を`emit(sendCancelParliamentFaction)`ですること
- 選択イベント送信を`emit(sendParliamentFactionLeastInterface,selectedDto)`ですること
