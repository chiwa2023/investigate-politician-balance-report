...
| 選挙区一識別コード           | electionDistrictCode | Integer | 変更にかかわらず選挙区が同一であることを識別するコード |
...
- キャンセルイベント送信を`emit(sendCancelElectionDistrict)`ですること
- 選択イベント送信を`emit(sendElectionDistrictLeastInterface,selectedDto)`ですること
