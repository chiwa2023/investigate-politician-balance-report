...
/**
 * RefleshYearDataAccessRepogitoryTasklet単体テスト
 */
@SpringJUnitConfig
...
class RefleshYearDataAccessRepositoryTaskletTest {
    ...
    /** 使途報告書格納フォルダ(Repository) */
    private static final String pathFunctionUsage = "main/java/mitei/mitei/investigate/report/balance/politician/repository/poli_party/usage/y";
    /** 使途報告書格納フォルダ(Repository) */
    private static final String pathFunctionBalancesheet = "main/java/mitei/mitei/investigate/report/balance/politician/repository/poli_org/balancesheet/y";

    @Test
    void testExecute() throws Exception { // NOPMD

        StepExecution execution = this.getStepExecution();
        refleshYearDataAccessRepositoryTasklet.beforeStep(execution);

        // 最後まで到達すればとりあえずOK(内容は各個別のTestを走らせるので)
        assertThat(refleshYearDataAccessRepositoryTasklet.execute(new StepContribution(execution), null))
                .isEqualTo(RepeatStatus.FINISHED);

        /* ファイルの存在だけ確認 */

        final String expandText = "Repository.java";
