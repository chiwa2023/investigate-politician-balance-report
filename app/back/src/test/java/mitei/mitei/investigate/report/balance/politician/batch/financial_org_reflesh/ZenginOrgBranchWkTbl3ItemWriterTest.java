...
/**
 * ZenginOrgBranchWkTbl3ItemWriter単体テスト
 */
@SpringJUnitConfig
@AutoConfigureMockMvc
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@DirtiesContext(classMode = ClassMode.BEFORE_CLASS)
class ZenginOrgBranchWkTbl3ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    @Tag("TableTruncate")
    @Transactional
    @Sql("truncate_zengin_org_branch_wk3.sql")
    void test() throws Exception { // NOPMD

        StepExecution execution = this.getStepExecution();
        zenginOrgBranchWkTbl3ItemWriter.beforeStep(execution);
        ...
        ZenginOrgBranchWk3Entity entity02 = new ZenginOrgBranchWk3Entity();
        ...
        List<ZenginOrgBranchWk3Entity> list = new ArrayList<>();
        list.add(entity01);
        list.add(entity02);

        // Chunkを作成してセット
        Chunk<? extends ZenginOrgBranchWk3Entity> items = new Chunk<>(list);
        zenginOrgBranchWkTbl3ItemWriter.write(items);

        List<ZenginOrgBranchWk3Entity> listAnswer = zenginOrgBranchWk3Repository.findAll();
        assertEquals(2, listAnswer.size(), "2件登録して件取得");
        listAnswer.sort((e1, e2) -> e1.getZenginOrgBranchWk3Code() - e2.getZenginOrgBranchWk3Code());

        ZenginOrgBranchWk3Entity answerEntity00 = listAnswer.get(0);
        ...
        ZenginOrgBranchWk3Entity answerEntity01 = listAnswer.get(1);
