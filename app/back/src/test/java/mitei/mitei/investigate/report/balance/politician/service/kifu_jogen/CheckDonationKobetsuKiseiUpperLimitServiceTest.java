...
/**
 * CheckDonationKobetsuKiseiUpperLimitServiceっ単体テスト
 */
@SpringJUnitConfig
...
class CheckDonationKobetsuKiseiUpperLimitServiceTest {
    // CHECKSTYLE:OFF

    /** テスト対象 */
    @Autowired
    private CheckDonationKobetsuKiseiUpperLimitService checkDonationKobetsuKiseiUpperLimitService;
    ...
    void testName() { // NOPMD
        ...
        KobetsuKifuJougenResultDto resultAllDto = checkDonationKobetsuKiseiUpperLimitService.practice(houkokuNen,
        ...
        SearchKifuJougenMeisaiBalancesheetResultDto tradingOrgDto = resultAllDto.getResultOrgDto();

        assertEquals(4, tradingOrgDto.getCountAll(), "合計4件取得");
        assertEquals(0, tradingOrgDto.getPageNumber(), "0ページで指定したのでそのまま"); // NOPMD

        List<KifuJougenTradingInfoDto> list = tradingOrgDto.getListTradingGroup();
        ...
        assertEquals(1, listMeisai00.size(), "1明細取得"); // NOPMD
        ...
        assertEquals(3, listMeisai01.size(), "3明細取得"); // NOPMD
        ...
        SearchKifuJougenMeisaiBalancesheetResultDto tradingPersonDto = resultAllDto.getResultPersonalDto();

        assertEquals(5, tradingPersonDto.getCountAll(), "合計5件取得");
        assertEquals(0, tradingPersonDto.getPageNumber(), "0ページで指定したのでそのまま");

        list = tradingPersonDto.getListTradingGroup();
    ...
    void testCode() { // NOPMD
        ...
        assertEquals(4, tradingOrgDto.getCountAll(), "合計4件取得");
        assertEquals(0, tradingOrgDto.getPageNumber(), "0ページで指定したのでそのまま");
        ...
        assertEquals(5, tradingPersonDto.getCountAll(), "合計4件取得");
        assertEquals(0, tradingPersonDto.getPageNumber(), "0ページで指定したのでそのまま");
