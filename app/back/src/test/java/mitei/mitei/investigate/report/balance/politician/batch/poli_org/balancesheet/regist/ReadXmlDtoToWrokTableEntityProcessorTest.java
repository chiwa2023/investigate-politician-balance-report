...
/**
 * ReadXmlDtoToWrokTableEntityProcessor単体テスト
 */
@SpringJUnitConfig
...
class ReadXmlDtoToWrokTableEntityProcessorTest {
    // CHECKSTYLE:OFF
   ...
   void test() throws Exception {
        ...
        detailEntity.setTaskPlanBalancesheetDetailId(756L);
        detailEntity.setTaskPlanBalancesheetDetailCode(750L);

        ReadXmlBalancesheetResultDto resultDto = unCompressedFileToReadXmlDtoProcessor.process(detailEntity);

        ReadXmlDtoToWrokTableEntityProcessor readXmlDtoToWrokTableEntityProcessor = new ReadXmlDtoToWrokTableEntityProcessor();

        WkTblPoliOrgBalancesheetReportEntity entity = readXmlDtoToWrokTableEntityProcessor.process(resultDto);

        // WkTblPoliOrgBalancesheetReportEntityからReadXmlBalancesheetResultDtoがBeanUtilsを用いて正しくコピーされるのは検証済

        /* 書証保存Dtoから抜粋 */
        assertThat(entity.getCharset()).isEqualTo(charset);
        assertThat(entity.getChildDir()).isEqualTo(childDir);
        assertThat(entity.getFileName()).isEqualTo(fileName);

        /* 表紙Dtoから抜粋 */
        assertThat(entity.getHoukokuNen()).isEqualTo(2022);
        assertThat(entity.getDaihyoushaNameFirst()).isEqualTo("孝志");
        assertThat(entity.getDaihyoushaNameLast()).isEqualTo("立花");

        /* 書類情報Dtoから抜粋 */
        assertThat(entity.getHoukokuNen()).isEqualTo(2022); // NOPMD
        assertThat(entity.getDantaiName()).isEqualTo("ホリエモン新党"); // NOPMD

        assertThat(entity.getTaskPlanBalancesheetDetailId()).isEqualTo(756L);
        assertThat(entity.getTaskPlanBalancesheetDetailCode()).isEqualTo(750L);
