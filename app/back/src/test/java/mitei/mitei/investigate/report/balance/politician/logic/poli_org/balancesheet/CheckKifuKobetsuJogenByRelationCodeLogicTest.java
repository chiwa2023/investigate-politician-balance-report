...
/**
 * CheckKifuKobetsuJogenByRelationCodeLogic単体テスト
 */
@SpringJUnitConfig
@AutoConfigureMockMvc
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@DirtiesContext(classMode = ClassMode.BEFORE_CLASS)
class CheckKifuKobetsuJogenByRelationCodeLogicTest {
    // CHECKSTYLE:OFF
    ...
    @Tag("TableTruncate")
    @Transactional
    @Sql({ "y2022/kobetsu_kisei_2022.sql", "kifu_souryou_kisei/political_organization.sql",
            "kifu_souryou_kisei/relation_person.sql" })
    void test() { // NOPMD

        Integer poliOrgCode = 101;

        KobetsuKifuJougenResultDto resultAllDto = checkKifuKobetsuJogenByRelationCodeLogic.practice(2022, poliOrgCode);
        ...
        List<KifuJougenTradingInfoDto> list = orgDto.getListTradingGroup();
        ...
        assertEquals("和歌山県実在市0", dto00.getPartnerAddress(), partnerAddressText);
        ...
        assertEquals("和歌山県実在市1", dto01.getPartnerAddress(), partnerAddressText);
        ...
        list = personDto.getListTradingGroup();
        assertEquals(3, list.size(), "3人取得");

        dto00 = list.get(0);
        assertEquals("個人寄付　太郎", dto00.getPartnerName(), partnerNameText);
        assertEquals("和歌山県実在市0", dto00.getPartnerAddress(), partnerAddressText);
        ...
        assertEquals("個人寄付　花子", dto01.getPartnerName(), partnerNameText);
        assertEquals("和歌山県実在市2", dto01.getPartnerAddress(), partnerAddressText);
        ...
        assertEquals("個人寄付　花子", dto02.getPartnerName(), partnerNameText);
