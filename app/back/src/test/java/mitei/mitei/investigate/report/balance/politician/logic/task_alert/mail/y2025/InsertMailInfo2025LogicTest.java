package mitei.mitei.investigate.report.balance.politician.logic.task_alert.mail.y2025;
...
/**
 * InsertMailInfo2025Logic単体テスト
 */
@SpringJUnitConfig
@AutoConfigureMockMvc
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@DirtiesContext(classMode = ClassMode.BEFORE_CLASS)
class InsertMailInfo2025LogicTest {

    /** テスト対象 */
    @Autowired
    private InsertMailInfo2025Logic insertMailInfo2025Logic;

    /** メール送信予定Repository(2025年) */
    @Autowired
    ...
    @Tag("TableTruncate")
    @Transactional
    @Sql({ "truncate_alert_mail_2025.sql", "../../../user_web_access/user_web_access.sql" })
    void testPractice() throws Exception { // NOPMD
        ...
        LocalDateTime shori = LocalDateTime.of(2025, 5, 22, 11, 22, 33);

        int size = insertMailInfo2025Logic.practice(privilegeDto, shori, userEntiy, listTask);

        assertThat(size).isEqualTo(2); // Taskを2件設定したので2行

        List<SendAlertMail2025Entity> listAlert = sendAlertMail2025Repository.findAll();
        ...
        SendAlertMail2025Entity result00 = listAlert.get(0);
        ...
        SendAlertMail2025Entity result01 = listAlert.get(1);
