...
import mitei.mitei.investigate.report.balance.politician.logic.poli_org.balancesheet.y2022.CopyBalancesheetWkTblIncomeY2022Logic;

/**
 * 収支報告書収入ワークテーブル複写用データ抽出Logic
 */
@Component
public class CopyBalancesheetWkTblIncomeLogic {
     ...
     * @return 検索結果
     */
    public List<WkTblBalancesheetIncomeEntity> practice(final int houkokuNen, final Long documentCode,
            final Integer pageNumber, final Integer chunkSize, final CheckPrivilegeDto privilegeDto) {

        switch (houkokuNen) {
            case YEAR_2022:
                return copyBalancesheetWkTblIncomeY2022Logic.practice(documentCode, pageNumber, chunkSize,
                        privilegeDto);
            default:
    ...
    public Integer practiceSize(final int houkokuNen, final Long documentCode) {
        switch (houkokuNen) {
            case YEAR_2022:
                return copyBalancesheetWkTblIncomeY2022Logic.practiceSize(documentCode);
            default:
                throw new IllegalArgumentException("Unexpected value: " + houkokuNen);
        }
    }
