 ...
 */
@Component
public class SaveMailSendingResult2022Logic {
    ...
    public SendMailAllPlanResultDto practice(final LocalDateTime datetTimeShori, final List<MailDataDto> listPickup,
        ...
        SendAlertMail2022Entity entity;
        Optional<SendAlertMail2022Entity> optional;

        List<SendAlertMail2022Entity> listUpdate = new ArrayList<>();
        // 送信試行リストはすべて更新=処理済に変更
        for (MailDataDto dto : listPickup) {
            optional = sendAlertMail2022Repository.findById(dto.getSendAlertMailId());
        ...
        List<SendAlertMail2022Entity> listInsert = new ArrayList<>();
        ...
        sendAlertMail2022Repository.saveAll(listUpdate);
        sendAlertMail2022Repository.saveAll(listInsert);
