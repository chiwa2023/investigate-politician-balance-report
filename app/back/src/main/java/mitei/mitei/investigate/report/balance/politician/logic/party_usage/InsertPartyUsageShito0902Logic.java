...
public class InsertPartyUsageShito0902Logic {
     ...
     */
    public int practice (final Long documentCode,final PartyUsageDocumentPoliticalPropertyDto partyUsageDocumentPoliticalPropertyDto, final Shito0902Dto shito0902Dto, final CheckPrivilegeDto checkPrivilegeDto) {

        List<OfferingPartyUsage0902Report2025Entity> list = new ArrayList<>();

        // 初期同一識別コードを取得
        Long code = 0L;
        Optional<OfferingPartyUsage0902Report2025Entity> optional = offeringPartyUsage0902Report2025Repository.findFirstByOrderByPartyUsage0902ReportCodeDesc();
        if(!optional.isEmpty()) {
        ...
        for(Sheet0902Dto dto : shito0902Dto.getList()) {
            code++;
            list.add(this.createEntity(code,documentCode, partyUsageDocumentPoliticalPropertyDto,dto,checkPrivilegeDto));
        }
        
        //repositoryで保存
        offeringPartyUsage0902Report2025Repository.saveAllAndFlush(list);
        
        return list.size();
    }

    private OfferingPartyUsage0902Report2025Entity createEntity(final Long code, final Long documentCode,final PartyUsageDocumentPoliticalPropertyDto partyUsageDocumentPoliticalPropertyDto,     final Sheet0902Dto dto, final CheckPrivilegeDto checkPrivilegeDto) {
