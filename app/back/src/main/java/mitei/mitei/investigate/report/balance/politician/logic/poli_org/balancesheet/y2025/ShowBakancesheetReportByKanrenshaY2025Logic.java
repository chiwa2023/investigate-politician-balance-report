 ...
 */
@Component
public class ShowBakancesheetReportByKanrenshaY2025Logic {

    /** 収支報告書収入テーブルRepository(2025) */
    @Autowired
    private OfferingBalancesheetIncome2025Repository offeringBalancesheetIncome2025Repository;

    /** 収支報告書支出テーブルRepository(2025) */
    @Autowired
    ...
    public IncomeAndOutcomeNaturalSearchResultDto practice(final KanrenshaBalancesheetConditionCapsuleDto capsuleDto) {
        ...
        if (capsuleDto.getAllCountIncome().equals(zero) && capsuleDto.getAllCountOutcome().equals(zero)) {
            // 件数取得処理
            resultDto.setCountIncome(offeringBalancesheetIncome2025Repository.findCountDataByPartnerCode(partnerKbn,
                    partnerCode, tuple6.getT1(), tuple6.getT2(), tuple6.getT3(), tuple6.getT4(), tuple6.getT5(),
                    tuple6.getT6()));
            resultDto.setCountOutcome(offeringBalancesheetOutcome2025Repository.findCountDataByPartnerCode(partnerKbn,
        ...
        if (resultDto.getCountIncome() > 0) {
            ...
            List<OfferingBalancesheetIncome2025Entity> listEntity = offeringBalancesheetIncome2025Repository
        ...
        if (resultDto.getCountIncome() > 0) {
            ...
            List<OfferingBalancesheetOutcome2025Entity> listEntity = offeringBalancesheetOutcome2025Repository
    ...
    private List<IncomeAndOutcomeSearchLineDto> createIncomeLine(
            final List<OfferingBalancesheetIncome2025Entity> listEntity) {

        List<IncomeAndOutcomeSearchLineDto> list = new ArrayList<>();

        for (OfferingBalancesheetIncome2025Entity entity : listEntity) {
    ...
    private List<IncomeAndOutcomeSearchLineDto> createOutcomeLine(
            final List<OfferingBalancesheetOutcome2025Entity> listEntity) {

        List<IncomeAndOutcomeSearchLineDto> list = new ArrayList<>();

        for (OfferingBalancesheetOutcome2025Entity entity : listEntity) {
    ...
    private IncomeAndOutcomeSearchLineDto createIncomeDto(final OfferingBalancesheetIncome2025Entity entity) {
        IncomeAndOutcomeSearchLineDto dto = new IncomeAndOutcomeSearchLineDto();
        BeanUtils.copyProperties(entity, dto);
        return dto;
    }

    private IncomeAndOutcomeSearchLineDto createOutcomeDto(final OfferingBalancesheetOutcome2025Entity entity) {
        IncomeAndOutcomeSearchLineDto dto = new IncomeAndOutcomeSearchLineDto();
        BeanUtils.copyProperties(entity, dto);
