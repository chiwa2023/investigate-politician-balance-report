...
import mitei.mitei.investigate.report.balance.politician.batch.financial_org_reflesh.AddFinancialTenpoWkTbl1ItemReader;
import mitei.mitei.investigate.report.balance.politician.batch.financial_org_reflesh.DeleteFinancialTenpoWkTbl1ItemReader;
import mitei.mitei.investigate.report.balance.politician.batch.financial_org_reflesh.MoveFinancialTenpoWkTbl1ItemReader;
...
import mitei.mitei.investigate.report.balance.politician.batch.financial_org_reflesh.ZenginOrgWKTbl1ChangeProcessor;
 ...
 */
@Configuration
public class PickupIdo1FinancialOrgTenpoConfiguration {

    /** 機能名 */
    private static final String FUNCTION_NAME = "pickupIdo1FinancialOrgTenpo";
    ...
    @Autowired
    private ZenginOrgWKTbl1ChangeProcessor zenginOrgWKTbl1ChangeProcessor;
    ...
    protected Step getStepAdd(final JobRepository jobRepository, final PlatformTransactionManager transactionManager) {
        ...
        delegates.add(zenginOrgWKTbl1ChangeProcessor);
        delegates.add(zenginChangeAddProcessor);

        CompositeItemProcessor<ZenginOrgBranchWk1Entity, ZenginOrgChangeBranchEntity> compositeItemProcessor = new CompositeItemProcessor<>();
        compositeItemProcessor.setDelegates(delegates);

        return new StepBuilder(STEP_ADD_NAME, jobRepository)
                .<ZenginOrgBranchWk1Entity, ZenginOrgChangeBranchEntity>chunk(CHUNK_SIZE, transactionManager)
                .reader(addFinancialTenpoWkTbl1ItemReader).processor(compositeItemProcessor)
    ...
    protected Step getStepMove(final JobRepository jobRepository, final PlatformTransactionManager transactionManager) {
        ...
        return new StepBuilder(STEP_MOVE_NAME, jobRepository)
                .<ZenginOrgBranchWk1Entity, ZenginOrgChangeBranchEntity>chunk(CHUNK_SIZE, transactionManager)
                .reader(moveFinancialTenpoWkTbl1ItemReader).processor(compositeItemProcessor)
                .writer(zenginOrgChangeBranchItemWriter).build();
    }
    ...
    protected Step getStepDelete(final JobRepository jobRepository,
        ...
        return new StepBuilder(STEP_DELETE_NAME, jobRepository)
                ...
                .reader(deleteFinancialTenpoWkTbl1ItemReader).processor(compositeItemProcessor)
                .writer(zenginOrgChangeBranchItemWriter).build();
    }
