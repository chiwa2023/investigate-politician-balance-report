...
import mitei.mitei.investigate.report.balance.politician.service.renketsu_koufukin.SearchRenketsuOutcomeFailureService;

/**
 * 支出交付金連結紐づけ失敗Controller
 */
@Controller
@RequestMapping("/renketsu-koufukin")
public class SearchRenketsuOutcomeFailureController extends AbstractTemplateCheckController {
    ...
    /** 交付金連結ワークテーブル作成Batch非同期実行Service */
    @Autowired
    ...
    @PostMapping("/outcome-failure")
    public ResponseEntity<RenketsuKoufukinFailureDto> practice(
            @RequestBody final TemplatePagingCapsuleDto capsuleDto) {
        ...
        try {
            ...
            return ResponseEntity.ok(searchRenketsuOutcomeFailureService.practice(capsuleDto));
