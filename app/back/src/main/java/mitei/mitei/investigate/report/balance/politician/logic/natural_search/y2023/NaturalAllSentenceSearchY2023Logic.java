package mitei.mitei.investigate.report.balance.politician.logic.natural_search.y2023;
...
/**
 * 政治資金収支報告書収入・支出検索(2023年)
 */
@Component
public class NaturalAllSentenceSearchY2023Logic {
    ...
    public IncomeAndOutcomeNaturalSearchResultDto practice(
        ...
        if (searchConditionDto.getIsSearchIncome()) {
            // 件数
            searchResultDto.setCountIncome(offeringBalancesheetIncome2023Repository.findFullTextCount(searchWords,
                    searchConditionDto.getStartDate(), searchConditionDto.getEndDate()));

            // 取得する実データ
            List<OfferingBalancesheetIncome2023Entity> listTemp = offeringBalancesheetIncome2023Repository.findFullText(
                    searchWords, searchConditionDto.getOffsetIncome(), searchConditionDto.getStartDate(),
                    searchConditionDto.getEndDate());

            for (OfferingBalancesheetIncome2023Entity entity : listTemp) {
        ...
        if (searchConditionDto.getIsSearchOutcome()) {
            searchResultDto.setCountOutcome(offeringBalancesheetOutcome2023Repository.findFullTextCount(searchWords,
                    searchConditionDto.getStartDate(), searchConditionDto.getEndDate()));

            // 取得する実データ
            List<OfferingBalancesheetOutcome2023Entity> listTemp = offeringBalancesheetOutcome2023Repository
                    .findFullText(searchWords, searchConditionDto.getOffsetIncome(), searchConditionDto.getStartDate(),
                            searchConditionDto.getEndDate());
            for (OfferingBalancesheetOutcome2023Entity entity : listTemp) {
     ...
     * @return 検索結果行Dto
     */
    private IncomeAndOutcomeSearchLineDto creatDto(final OfferingBalancesheetOutcome2023Entity entity) {
     ...
     * @return 検索結果行Dto
     */
    private IncomeAndOutcomeSearchLineDto creatDto(final OfferingBalancesheetIncome2023Entity entity) {
        IncomeAndOutcomeSearchLineDto lineDto = new IncomeAndOutcomeSearchLineDto();
        BeanUtils.copyProperties(entity, lineDto);
