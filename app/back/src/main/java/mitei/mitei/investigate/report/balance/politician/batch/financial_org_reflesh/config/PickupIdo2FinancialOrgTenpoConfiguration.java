...
import mitei.mitei.investigate.report.balance.politician.batch.financial_org_reflesh.AddFinancialTenpoWkTbl2ItemReader;
import mitei.mitei.investigate.report.balance.politician.batch.financial_org_reflesh.DeleteFinancialTenpoWkTbl2ItemReader;
import mitei.mitei.investigate.report.balance.politician.batch.financial_org_reflesh.MoveFinancialTenpoWkTbl2ItemReader;
...
import mitei.mitei.investigate.report.balance.politician.batch.financial_org_reflesh.ZenginOrgWKTbl2ChangeProcessor;
 ...
 */
@Configuration
public class PickupIdo2FinancialOrgTenpoConfiguration {

    /** 機能名 */
    private static final String FUNCTION_NAME = "pickupIdo2FinancialOrgTenpo";
    ...
    @Autowired
    private ZenginOrgWKTbl2ChangeProcessor zenginOrgWKTbl2ChangeProcessor;
    ...
    protected Step getStepAdd(final JobRepository jobRepository, final PlatformTransactionManager transactionManager) {
        ...
        delegates.add(zenginOrgWKTbl2ChangeProcessor);
        delegates.add(zenginChangeAddProcessor);

        CompositeItemProcessor<ZenginOrgBranchWk2Entity, ZenginOrgChangeBranchEntity> compositeItemProcessor = new CompositeItemProcessor<>();
        compositeItemProcessor.setDelegates(delegates);

        return new StepBuilder(STEP_ADD_NAME, jobRepository)
                .<ZenginOrgBranchWk2Entity, ZenginOrgChangeBranchEntity>chunk(CHUNK_SIZE, transactionManager)
                .reader(addFinancialTenpoWkTbl2ItemReader).processor(compositeItemProcessor)
    ...
    protected Step getStepMove(final JobRepository jobRepository, final PlatformTransactionManager transactionManager) {
        ...
        return new StepBuilder(STEP_MOVE_NAME, jobRepository)
                .<ZenginOrgBranchWk2Entity, ZenginOrgChangeBranchEntity>chunk(CHUNK_SIZE, transactionManager)
                .reader(moveFinancialTenpoWkTbl2ItemReader).processor(compositeItemProcessor)
                .writer(zenginOrgChangeBranchItemWriter).build();
    }
    ...
    protected Step getStepDelete(final JobRepository jobRepository,
        ...
        return new StepBuilder(STEP_DELETE_NAME, jobRepository)
                ...
                .reader(deleteFinancialTenpoWkTbl2ItemReader).processor(compositeItemProcessor)
                .writer(zenginOrgChangeBranchItemWriter).build();
    }
