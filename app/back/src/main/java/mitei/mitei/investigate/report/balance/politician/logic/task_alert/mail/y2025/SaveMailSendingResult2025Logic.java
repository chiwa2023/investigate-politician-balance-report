 ...
 */
@Component
public class SaveMailSendingResult2025Logic {
    ...
    public SendMailAllPlanResultDto practice(final LocalDateTime datetTimeShori, final List<MailDataDto> listPickup,
        ...
        SendAlertMail2025Entity entity;
        Optional<SendAlertMail2025Entity> optional;

        List<SendAlertMail2025Entity> listUpdate = new ArrayList<>();
        // 送信試行リストはすべて更新=処理済に変更
        for (MailDataDto dto : listPickup) {
            optional = sendAlertMail2025Repository.findById(dto.getSendAlertMailId());
        ...
        List<SendAlertMail2025Entity> listInsert = new ArrayList<>();
        ...
        sendAlertMail2025Repository.saveAll(listUpdate);
        sendAlertMail2025Repository.saveAll(listInsert);
