...
public interface OfferingBalancesheetIncome2022Repository
    ...
    List<OfferingBalancesheetIncome2022Entity> findFullText(String searchWords, int offset, LocalDate startDate,
            LocalDate endDate);
     ...
     * @return 件数
     */
    @Query(value = "SELECT count(*) AS count FROM offering_balancesheet_income_2022 WHERE saishin_kbn= 1 AND accrual_date_value BETWEEN ?2 AND ?3 AND MATCH(search_words) AGAINST (?1 IN BOOLEAN MODE)", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT DISTINCT relation_person_code_income FROM offering_balancesheet_income_2022 WHERE political_organization_code=?1 AND youshiki_kbn=?2 AND youshiki_eda_kbn=1 and saishin_kbn = 1", nativeQuery = true)
     ...
     * @return 取引相手関連者同一識別コードリスト
     */
    @Query(value = "SELECT DISTINCT relation_corp_code_income FROM offering_balancesheet_income_2022 WHERE political_organization_code=?1 AND youshiki_kbn=?2 AND youshiki_eda_kbn=2 and saishin_kbn = 1 ", nativeQuery = true)
     ...
     * @return 取引相手関連者同一識別コードリスト
     */
    @Query(value = "SELECT DISTINCT relation_political_org_code_income FROM offering_balancesheet_income_2022 WHERE political_organization_code=?1 AND youshiki_kbn=?2 AND youshiki_eda_kbn=3 and saishin_kbn = 1 ", nativeQuery = true)
     ...
     * @return 収入データリスト
     */
    List<OfferingBalancesheetIncome2022Entity> findByPoliticalOrganizationCodeInAndYoushikiKbnInAndSaishinKbn(
     ...
     * @return 件数
     */
    @Query(value = "SELECT count(distinct trading_partner_code)" + " FROM offering_balancesheet_income_2022" // NOPMD
     ...
     * @return 件数
     */
    @Query(value = "SELECT count(distinct partner_name,partner_juusho)" + " FROM offering_balancesheet_income_2022"
    ...
    @Query(value = "SELECT sum(kingaku) AS sum , trading_partner_code AS relation_code"
            + " FROM offering_balancesheet_income_2022"
            + " WHERE political_organization_code= ?1 AND youshiki_kbn = 11 AND trading_partner_code <> 0"
     ...
     * @return 検索結果
     */
    List<OfferingBalancesheetIncome2022Entity> findByPoliticalOrganizationCodeAndTradingPartnerCodeAndYoushikiKbnAndSaishinKbn(
     ...
     * @return 検索結果
     */
    @Query(value = "SELECT COUNT(*) FROM offering_balancesheet_income_2022" // 改行調整
     ...
     * @return 検索結果
     */
    @Query(value = "SELECT * FROM offering_balancesheet_income_2022" // 改行調整
            + "  WHERE trading_partner_Kbn = ?1 AND trading_partner_code = ?2"
            + "    AND ( (youshiki_kbn IN ?3 AND youshiki_eda_kbn IN ?4 ) OR"
            + "          (youshiki_kbn IN ?5 AND youshiki_eda_kbn IN ?6 ) OR"
            + "          (youshiki_kbn IN ?7 AND youshiki_eda_kbn IN ?8 ) ) AND saishin_kbn = 1 ", nativeQuery = true)
    List<OfferingBalancesheetIncome2022Entity> findDataByPartnerCode(Integer partnerKbn, Integer partnerCode,
            List<Integer> list1, List<Integer> list2, List<Integer> list3, List<Integer> list4, List<Integer> list5,
