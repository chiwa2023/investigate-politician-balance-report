 ...
 */
@Configuration
public class UkaiKenkinRouteByStage04BatchConfiguration {

    /** チャンクサイズ */
    private static final int CHUNK_SIZE = 50;

    /** 機能名 */
    private static final String FUNCTION_NAME = "ukaiKenkinRouteByStage04";
    ...
    protected Job getJob(final JobRepository jobRepository, @Qualifier(STEP_CLEAN_NAME) final Step stepClean,
            ...
            @Qualifier(STEP_STAGE_4) final Step stepStage4,
        ...
        return new JobBuilder(JOB_NAME, jobRepository).incrementer(new RunIdIncrementer()).flow(stepClean)
                .next(stepStage0).next(stepStage1).next(stepStage2).next(stepStage3).next(stepStage4) // ここまで階層
                .next(stepPickupZero)
    ...
    protected Step getRouteCorp(final JobRepository jobRepository,
        ...
        return new StepBuilder(STEP_ROUTE_CORP, jobRepository).tasklet(pickupUkaiKenkinCorpTasklet, transactionManager)
                .build();
    }
