 ...
 */
public interface OfferingBalancesheetOutcome2025Repository
        extends JpaRepository<OfferingBalancesheetOutcome2025Entity, Integer> {
    ...
    List<OfferingBalancesheetOutcome2025Entity> findFullText(String searchWords, int offset, LocalDate startDate,
            LocalDate endDate);
     ...
     * @return 件数
     */
    @Query(value = "SELECT count(*) AS count FROM offering_balancesheet_outcome_2025 WHERE saishin_kbn= 1 AND accrual_date_value BETWEEN ?2 AND ?3 AND MATCH(search_words) AGAINST (?1 IN BOOLEAN MODE)", nativeQuery = true)
     ...
     * @return データリスト
     */
    List<OfferingBalancesheetOutcome2025Entity> findByDocumentCodeOrderByOfferingBalancesheetOutcomeId(
            Long documentCode);
     ...
     * @return 検索結果
     */
    List<OfferingBalancesheetOutcome2025Entity> findByRelationPoliticalOrgCodeOutcomeAndSaishinKbnAndPoliticalOrganizationCodeIn(
