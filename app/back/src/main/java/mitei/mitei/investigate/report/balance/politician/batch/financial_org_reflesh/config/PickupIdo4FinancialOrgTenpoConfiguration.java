...
import mitei.mitei.investigate.report.balance.politician.batch.financial_org_reflesh.AddFinancialTenpoWkTbl4ItemReader;
import mitei.mitei.investigate.report.balance.politician.batch.financial_org_reflesh.DeleteFinancialTenpoWkTbl4ItemReader;
import mitei.mitei.investigate.report.balance.politician.batch.financial_org_reflesh.MoveFinancialTenpoWkTbl4ItemReader;
...
import mitei.mitei.investigate.report.balance.politician.batch.financial_org_reflesh.ZenginOrgWKTbl4ChangeProcessor;
 ...
 */
@Configuration
public class PickupIdo4FinancialOrgTenpoConfiguration {

    /** 機能名 */
    private static final String FUNCTION_NAME = "pickupIdo4FinancialOrgTenpo";
    ...
    @Autowired
    private ZenginOrgWKTbl4ChangeProcessor zenginOrgWKTbl4ChangeProcessor;
    ...
    protected Step getStepAdd(final JobRepository jobRepository, final PlatformTransactionManager transactionManager) {
        ...
        delegates.add(zenginOrgWKTbl4ChangeProcessor);
        delegates.add(zenginChangeAddProcessor);

        CompositeItemProcessor<ZenginOrgBranchWk4Entity, ZenginOrgChangeBranchEntity> compositeItemProcessor = new CompositeItemProcessor<>();
        compositeItemProcessor.setDelegates(delegates);

        return new StepBuilder(STEP_ADD_NAME, jobRepository)
                .<ZenginOrgBranchWk4Entity, ZenginOrgChangeBranchEntity>chunk(CHUNK_SIZE, transactionManager)
                .reader(addFinancialTenpoWkTbl4ItemReader).processor(compositeItemProcessor)
    ...
    protected Step getStepMove(final JobRepository jobRepository, final PlatformTransactionManager transactionManager) {
        ...
        return new StepBuilder(STEP_MOVE_NAME, jobRepository)
                .<ZenginOrgBranchWk4Entity, ZenginOrgChangeBranchEntity>chunk(CHUNK_SIZE, transactionManager)
                .reader(moveFinancialTenpoWkTbl4ItemReader).processor(compositeItemProcessor)
                .writer(zenginOrgChangeBranchItemWriter).build();
    }
    ...
    protected Step getStepDelete(final JobRepository jobRepository,
        ...
        return new StepBuilder(STEP_DELETE_NAME, jobRepository)
                ...
                .reader(deleteFinancialTenpoWkTbl4ItemReader).processor(compositeItemProcessor)
                .writer(zenginOrgChangeBranchItemWriter).build();
    }
