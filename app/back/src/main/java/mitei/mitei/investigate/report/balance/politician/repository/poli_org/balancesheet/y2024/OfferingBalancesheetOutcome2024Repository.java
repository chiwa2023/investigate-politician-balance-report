...
public interface OfferingBalancesheetOutcome2024Repository
     ...
     * @return 検索結果
     */
    @Query(value = "SELECT COUNT(*) FROM offering_balancesheet_outcome_2024" // 改行調整
     ...
     * @return 検索結果
     */
    @Query(value = "SELECT * FROM offering_balancesheet_outcome_2024" // 改行調整
            + "  WHERE trading_partner_Kbn = ?1 AND trading_partner_code = ?2"
            + "    AND ( (youshiki_kbn IN ?3 AND youshiki_eda_kbn IN ?4 ) OR"
            + "          (youshiki_kbn IN ?5 AND youshiki_eda_kbn IN ?6 ) ) AND saishin_kbn = 1 ", nativeQuery = true)
    List<OfferingBalancesheetOutcome2024Entity> findDataByPartnerCode(Integer partnerKbn, Integer partnerCode,
