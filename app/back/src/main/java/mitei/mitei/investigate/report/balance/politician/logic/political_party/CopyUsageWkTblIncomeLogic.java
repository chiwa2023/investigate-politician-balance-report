...
import mitei.mitei.investigate.report.balance.politician.logic.party_usage.y2022.CopyUsageWkTblIncomeY2022Logic;

/**
 * 使途報告書ワークテーブル複写用データ抽出ファクトリメソッドLogic
 */
@Component
public class CopyUsageWkTblIncomeLogic {
     ...
     * @return 検索結果
     */
    public List<WkTblUsageIncomeEntity> practice(final int houkokuNen, final Long documentCode,
            final Integer pageNumber, final Integer chunkSize, final CheckPrivilegeDto privilegeDto) {

        switch (houkokuNen) {
            case YEAR_2022:
                return copyUsageWkTblIncomeY2022Logic.practice(documentCode, pageNumber, chunkSize, privilegeDto);
            default:
    ...
    public Integer practiceSize(final int houkokuNen, final Long documentCode) {
        switch (houkokuNen) {
            case YEAR_2022:
                return copyUsageWkTblIncomeY2022Logic.practiceSize(documentCode);
            default:
                throw new IllegalArgumentException("Unexpected value: " + houkokuNen);
        }
    }
