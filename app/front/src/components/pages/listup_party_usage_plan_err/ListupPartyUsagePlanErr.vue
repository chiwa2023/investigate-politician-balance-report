...
function onSearchDetail() {
    ...
    // エラーリスリストを取得(文字コード部分)
    const url = "http://localhost:9080/listup-party-usage-charset/error";
    const method = "POST";
...
function onsSearchPrepared() {
    ...
    // エラーリスリストを取得(政治団体部分)
    const url = "http://localhost:9080/listup-party-usage-poli-org/error";
    const method = "POST";
    const body = JSON.stringify(capsuleDto);
    const headers = {
        'Accept': 'application/json',
...
<template>
    <h1>政党交付金使途報告書解析予定エラーリスト</h1>

    <div class="one-line">
        使途報告書の解析未処理リストを一覧しています({{ listDetail.length }}件)
        <table>
            <tr>
                <th>保存時間</th>
                <th>ファイル名</th>
    ...
    <hr>
    <div class="one-line">
        使途報告書の解析未処理リストを一覧しています(政治団体未定)({{ listPrepared.length }}件)
        <table>
            <tr>
                <th>登録時間</th>
                <th>報告年</th>
