...
npm audit fix またはvite全体は npm i vite
...
総務省から配布されている公式ソフトウェアから生成される形式XMLの読み書きに使用する機能は、別プロジェクトで作成しており、認識できない場合は `mvnw clean` でinstallが必要

最初の1回のみapprication.propertiesの編集

1. MYSQL接続設定

2. アップロードファイル保存ディレクトリの指定
